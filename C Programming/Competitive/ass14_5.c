/*
Algorithm:

    START
        Accept number as Arr[]
        Accept size as iSize
        Accept another number as iNo 
        Perform to return frequency of another number
        Display the result
    STOP
*/

////////////////////////////////////////////////////////////////////////////////////////////////////////////
//
// Include required header files
//
////////////////////////////////////////////////////////////////////////////////////////////////////////////

#include<stdio.h>
#include<stdlib.h>
#include<stdbool.h>

////////////////////////////////////////////////////////////////////////////////////////////////////////////
//
// Function name    :   Frequency
// Input    :           int, int, int
// Output   :           int
// Description  :       Perform to return frequency of another number
// Date :               04/06/2026 
// Author   :           Sharvari Gorakhnath Bhosale
//
////////////////////////////////////////////////////////////////////////////////////////////////////////////


int Frequency   (
                    int Arr[],                      //input
                    int iSize,                      //size of input
                    int iNo                         //another input
                )
{
    int iCnt = 0;                                   //initialize the counter
    int iCount = 0;

    for(iCnt = 0; iCnt < iSize; iCnt++)
    {
        if(Arr[iCnt] == iNo)
        {
            iCount++;
        }
    }
    return iCount;
}

////////////////////////////////////////////////////////////////////////////////////////////////
//
// Application to perform to return frequency of another number of 3 int values
//
////////////////////////////////////////////////////////////////////////////////////////////////


int main()
{
    int *P = NULL;                                  //initialize NULL pointer
    int iLength = 0;                                //variable to store size of input
    int iCnt = 0;                                   //initialize counter
    int iValue = 0;                                 //variable to store another input
    int iRet = 0;                                   //variable to store return value

    printf("Enter number of element : \n");         //accept number
    scanf("%d",&iLength);

    P = (int *) malloc (iLength * sizeof(int));     //allocate the memory

    if(P == NULL)
    {
        printf("Unable to allocate memory");
        return -1;
    }

    printf("Enter elements : \n");                  //accept elements
    for(iCnt = 0; iCnt < iLength; iCnt++)
    {
        scanf("%d",&P[iCnt]);
    }

    printf("Enter number that you want to search : \n");
    scanf("%d",&iValue);

    iRet = Frequency(P, iLength, iValue);

    printf("Frequency is : %d",iRet);               //display result

    free(P);                                        //deallocate memory

    return 0;
}

////////////////////////////////////////////////////////////////////////////////////////////////
//
//Input :   N:  6
//          NO: 66
//          Elements:   85 66 3 66 93 88
//Output:   2
//
//Input:    N:  6
//          NO: 12
//          Elements:   85 11 3 15 11 111
//Output:   0
//
////////////////////////////////////////////////////////////////////////////////////////////////